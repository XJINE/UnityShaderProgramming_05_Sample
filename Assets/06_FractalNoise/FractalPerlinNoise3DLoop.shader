Shader "Sample/FractalPerlinNoise3DLoop"
{
    Properties
    {
    ...
    SubShader
    {
        Pass
        {
            ...
            float perlinNoise3DLoop(float3 seeds, float3 scale)
            {
                scale  = floor(scale);
                seeds *= scale;
                ...
                bool3  loop   = i == (scale - 1);
                float3 offset = float3(loop.x ? -i.x : 1,
                                       loop.y ? -i.y : 1,
                                       loop.z ? -i.z : 1);

                float3 i000 = i + float3(       0,        0,        0);
                float3 i100 = i + float3(offset.x,        0,        0);
                float3 i010 = i + float3(       0, offset.y,        0);
                float3 i110 = i + float3(offset.x, offset.y,        0);
                float3 i001 = i + float3(       0,        0, offset.z);
                float3 i101 = i + float3(offset.x,        0, offset.z);
                float3 i011 = i + float3(       0, offset.y, offset.z);
                float3 i111 = i + float3(offset.x, offset.y, offset.z);
            ...
            float fractalPerlinNoise3DLoop(float3 seeds, float3 scale, int octaves)
            {
                float value     = 0.0;
                float amplitude = 0.5;
                ...
                {
                    value += perlinNoise3DLoop(seeds, scale) * amplitude;
            ...
            {
                float3 scale    = _NoiseScale;
                       scale.x *= _NoiseAspect;

                return fractalPerlinNoise3DLoop(float3(i.uv, _Time.x), scale, _NoiseOctaves);
            }
